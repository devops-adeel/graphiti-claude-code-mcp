...
class LangfuseTrace(BaseModel):
    ...
    def average_score(self) -> Optional[float]:
        ...
        if not self.scores:
            return None
        numeric_scores = [
            s.value for s in self.scores if s.data_type == ScoreDataType.NUMERIC
        ]
...
class AnalysisResult(BaseModel):
    ...
    def has_issues(self) -> bool:
        ...
        return (
            self.total_errors > 0
            or self.state_losses > 0
            or len(self.patterns_detected) > 0
        )
...
class TestFailureAnalysis(BaseModel):
    ...
    def has_critical_issues(self) -> bool:
        ...
        return self.error_count > 3 or any(
            "state_lost" in str(transition) for transition in self.state_transitions
        )
    ...
    def get_error_summary(self) -> str:
        ...
        summary += ", ".join(
            f"{count} in {type}" for type, count in error_types.items()
        )
        return summary
