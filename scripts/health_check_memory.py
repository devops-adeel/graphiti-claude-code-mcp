...
class HealthCheck:
    ...
    async def check_storage(self) -> bool:
        ...
        try:
            ...
            found = any(
                self.test_memory_id in str(getattr(r, 'uuid', ''))
                for r in results
            )
            ...
            if self.fix:
                ...
                print(f"  GRAPH.QUERY {os.getenv('FALKORDB_DATABASE', 'shared_gtd_knowledge')}")
    ...
    async def check_temporal(self) -> bool:
        ...
        try:
            ...
            for days in test_ages:
                ...
                color = "success" if weight > 0.8 else "warning" if weight > 0.5 else "error"
                
                self.log(f"Age {days:3d} days: weight = {weight:.3f} ({status})", color)
            
            # Show cutoff point
            cutoff_days = int((-0.5) / (0.95 - 1)) if decay_factor == 0.95 else 30
            self.log(f"Memories become 'weak' (< 0.5) after ~{cutoff_days} days", "info")
    ...
    async def check_cross_domain(self) -> bool:
        ...
        try:
            ...
            cross_ref_enabled = os.getenv('ENABLE_CROSS_REFERENCES', 'true').lower() == 'true'
            
            self.log(f"GTD Integration: {'enabled' if gtd_enabled else 'disabled'}", 
                    "success" if gtd_enabled else "warning")
            self.log(f"Cross References: {'enabled' if cross_ref_enabled else 'disabled'}",
    ...
    async def run(self) -> int:
        ...
        print(f"\n{BLUE}{'='*60}{NC}")
        if passed == total:
            print(f"{GREEN}✓ All {total} checks passed - Memory pipeline is healthy!{NC}")
            return 0
        else:
