...
from langfuse_integration.langfuse_analyzer import (
    LangfuseAnalyzer,
    get_langfuse_analyzer,
)
from langfuse_integration.langfuse_patterns import PatternDetector, PatternType
...
class TestLangfuseE2EIntegration:
    ...
    @pytest.mark.asyncio
    async def test_state_loss_detection_and_storage(
        self, analyzer_with_mock, shared_memory
    ):
    ...
    async def test_pattern_signature_generation(self):
        ...
        sig1 = detector.generate_signature(
            PatternType.STATE_LOSS.value, from_phase="planning", to_phase="review"
        )
        ...
        sig3 = detector.generate_signature(
            PatternType.INTERRUPT.value, tool="check_in", context="test"
        )
    ...
    async def test_memory_supersession(self, shared_memory):
        ...
        with patch.object(shared_memory, "supersede_memory") as mock_supersede:
            ...
            with patch.object(
                shared_memory, "search_with_temporal_weight"
            ) as mock_search:
    ...
    async def test_temporal_weight_in_search(self, shared_memory):
        ...
        with patch.object(
            shared_memory, "_apply_temporal_weight", return_value=0.75
        ) as mock_weight:
            with patch.object(
                shared_memory, "search", new_callable=AsyncMock
            ) as mock_search:
                mock_search.return_value = [
                    type(
                        "Result",
                ...
                shared_memory.search_with_temporal_weight.side_effect = (
                    mock_search_with_weight
                )
    ...
    async def test_cross_domain_insights(self, shared_memory):
        ...
        with patch.object(shared_memory, "search_with_temporal_weight") as mock_search:
            ...
            assert any(
                "langfuse" in str(i.metadata.get("source", "")) for i in insights
            )
    ...
    async def test_test_failure_analysis(self, analyzer_with_mock):
        ...
        result = await analyzer_with_mock.analyze_test_failure(
            session_id=session_id, return_patterns=True
        )
        ...
        if "patterns" in result:
            ...
            error_patterns = [
                p for p in result["patterns"] if p["type"] == "test_error"
            ]
    ...
    async def test_prediction_based_on_history(self, analyzer_with_mock, shared_memory):
        ...
        with patch.object(shared_memory, "search_with_temporal_weight") as mock_search:
            ...
            result = await analyzer_with_mock.predict_trace_issues(
                trace_id=trace_id, threshold=0.7
            )
    ...
    async def test_complete_pipeline_flow(self, analyzer_with_mock, shared_memory):
        ...
        with patch.object(shared_memory, "add_memory") as mock_add:
            mock_add.side_effect = (
                lambda data, source: stored_memories.append(data)
                or f"memory-{len(stored_memories)}"
            )
