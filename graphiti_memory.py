...
class SearchResultWrapper:
    ...
    def __init__(
        self, result: Any, computed_score: float = None, metadata: dict = None
    ):
...
class SharedMemory:
    ...
    async def search_with_batching(self, query: str, results: List[Any]) -> List[Any]:
        """
        ...
        if batches:
            logger.info(
                f"Split results into {len(batches)} batches to fit token limits"
            )
                ...
                if len(parts) >= 2:
                    ...
                    if major == 0 and minor == 17:
                        if patch >= 9 and patch <= 10:
                            ...
                            logger.warning(
                                f"Graphiti v{version} is untested but may work"
                            )
                    else:
                        logger.warning(
                            f"Graphiti v{version} is not tested with this memory layer"
                        )
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Connected to shared knowledge graph: {self.database}/{self.group_id}"
            )
    ...
    async def supersede_memory(
        self, old_id: str, new_content: dict, reason: str
    ) -> str:
        """
        ...
        new_id = await self.add_memory(
            new_content, new_content.get("source", "claude_code")
        )
    ...
    async def mark_historical(self, memory_id: str, days_old: int = 30) -> None:
        ...
        logger.info(
            f"Memory {memory_id} is {days_old} days old (historical status tracked via age)"
        )

    async def search_with_temporal_weight(
        self, query: str, include_historical: bool = None, filter_source: str = None
    ) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Search failed with query '{query}' (escaped: '{safe_query}'): {e}"
            )
            ...
            if query != safe_query:
                ...
                results = await self.client.search(
                    simplified_query, group_ids=[self.group_id], num_results=30
                )
            else:
                raise
        ...
        if total_tokens > self.max_tokens:
            logger.info(
                f"Results exceed token limit ({total_tokens} > {self.max_tokens}). Applying batching..."
            )
                    ...
                    insights.append(
                        {
                            ...
                            "score": getattr(
                                result, "final_score", getattr(result, "score", 0.5)
                            ),
    ...
    async def link_to_gtd_task(self, memory_id: str, task_id: str):
        ...
        logger.info(
            f"GTD link tracked via metadata: memory {memory_id} -> task {task_id}"
        )
    ...
    async def build_smart_index(self) -> Dict[str, List]:
        ...
        for pattern_name, pattern_regex in patterns.items():
            try:
                # Search for pattern
                results = await self.client.search(
                    pattern_regex, group_ids=[self.group_id], num_results=100
                )
                ...
                logger.info(
                    f"Indexed {len(processed)} items for pattern: {pattern_name}"
                )
    ...
    async def _cache_pattern_results(
        self, pattern_name: str, results: List[Dict], ttl: int = 3600
    ):
        """
    ...
    async def search_with_smart_index(self, query: str) -> List[Any]:
        ...
        pattern_matches = {
            ...
            "command_patterns": any(
                word in query_lower for word in ["docker", "git", "npm", "pytest"]
            ),
            "solution_patterns": any(
                word in query_lower for word in ["fix", "solution", "resolve"]
            ),
            "tdd_patterns": any(word in query_lower for word in ["test", "tdd"]),
            "deployment_patterns": any(
                word in query_lower for word in ["deploy", "deployment"]
            ),
        }
    ...
    async def _get_cached_pattern(self, pattern_name: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            results = await self.client.search(
                cache_key, group_ids=[self.group_id], num_results=1
            )
                ...
                if hasattr(result, "episode_body"):
                    try:
                        ...
                        cached_at = datetime.fromisoformat(
                            cache_data.get("cached_at", "").replace("Z", "+00:00")
                        )
                        age_minutes = (
    ...
    def _filter_cached_by_query(
        self, cached_results: List[Dict], query: str
    ) -> List[Any]:
        """
    ...
    async def optimize_memory_graph(self):
        ...
        all_memories = await self.client.search(
            "*", group_ids=[self.group_id], num_results=1000  # Get all memories
        )
            ...
            if hasattr(memory, "episode_body"):
                try:
                    ...
                    if "timestamp" in metadata:
                        ...
                        if (
                            age_days > 30
                            and metadata.get("status") == MemoryStatus.ACTIVE.value
                        ):
                            await self.mark_historical(
                                getattr(memory, "id", "unknown"), age_days
                            )
                            historical_count += 1
                except Exception as e:
