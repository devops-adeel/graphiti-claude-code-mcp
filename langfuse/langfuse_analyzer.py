...
class LangfuseAnalyzer:
    ...
    def __init__(self):
        ...
        if not self.public_key or not self.secret_key:
            ...
            self.mcp_tags = [os.getenv("MCP_TRACE_TAG", "mcp-internal"), 
    ...
    def _should_skip_trace(self, trace) -> bool:
        ...
        if hasattr(trace, 'metadata') and trace.metadata:
            if isinstance(trace.metadata, dict) and trace.metadata.get("source") == self.mcp_metadata["source"]:
                return True
    ...
    async def analyze_recent_traces(
        ...
        if skipped_count > 0:
            logger.info(f"Filtered out {skipped_count} MCP-internal traces to prevent loops")
        ...
        for trace in filtered_traces:
            ...
            observations_response = self.client.api.observations.get_many(trace_id=trace.id)
            # Extract data from response
            observations = observations_response.data if hasattr(observations_response, 'data') else observations_response
            ...
            scores = scores_response.data if hasattr(scores_response, 'data') else scores_response
            if scores:
            ...
            for obs in observations if observations else []:
                ...
                if self._is_interrupt_observation(obs):
                    ...
                    pattern = {
                        ...
                        "tool": obs.name,
                        "timestamp": obs.start_time.isoformat() if obs.start_time else None,
                        "confidence": 0.95
                    }
            ...
            for pattern in similar_patterns:
                if pattern.score >= threshold:
                    predictions.append({
                        ...
                        "previous_occurrences": pattern.metadata.get("occurrences", 1)
                    })
    ...
    def _is_interrupt_observation(self, obs: Any) -> bool:
        ...
        if obs.name and any(pattern in obs.name.lower() for pattern in 
                           ["check_in", "wait_for", "confirm_with", "interrupt"]):
            return True
    ...
    async def _extract_trace_features(
        ...
        # Trace name pattern
        if trace.name:
            features["trace_type"] = trace.name.split("-")[0] if "-" in trace.name else trace.name
        ...
        if transitions:
            ...
            for trans in transitions:
                features[f"transition_{trans['from_phase']}_{trans['to_phase']}"] = "true"
