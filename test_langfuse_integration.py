...
async def test_pattern_detector():
    ...
    try:
        ...
        signatures = [
            detector.generate_signature("state_loss", from_phase="planning", to_phase="review"),
        ...
        from langfuse_patterns import PatternType
...
async def test_models():
    ...
    try:
        ...
        print(f"✓ Created pattern with confidence level: {pattern.confidence_level.value}")
        ...
        print(f"✓ Created analysis result with {result.unique_patterns} unique patterns")
...
async def test_recent_traces_analysis():
    ...
    try:
        ...
        if result.get('traces_analyzed', 0) == 0:
            print("  ℹ No traces found in the last hour (this is normal if no activity)")
